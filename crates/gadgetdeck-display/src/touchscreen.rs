...
impl TouchscreenStrip {
    ...
    pub fn on_touch_end(&mut self, rel_x: u16, rel_y: u16) -> Option<TouchGesture> {
        let result = if let (Some((start_x, start_y)), Some(start_time)) =
            (self.touch_start, self.touch_start_time)
        {
    ...
    pub fn update_image(
        ...
        let mut loaded_image = unsafe {
            ...
            if img.data.is_null() {
                log::warn!(
                    "Failed to load LCD JPEG segment ({} bytes)",
                    image_data.len()
                );
                return;
            }
