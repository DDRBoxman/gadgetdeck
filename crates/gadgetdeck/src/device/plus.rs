...
pub enum KnobEvent {
    ...
    Turn { knob: KnobIndex, direction: KnobDirection, steps: u8 },
}
...
impl KnobEvent {
    ...
    pub fn build_input_report(&self) -> Vec<u8> {
        ...
        match self {
            ...
            KnobEvent::Turn { knob, direction, steps } => {
...
impl PlusInputState {
    ...
    pub fn send_touch(&self, event: TouchEvent) {
        self.touch_event_type.store(event.event_type as u8, Ordering::Relaxed);
        ...
        log::info!("Touch event queued: {:?} at ({}, {})", event.event_type, event.x, event.y);
    }
    ...
    pub fn turn_knob(&self, knob: KnobIndex, steps: i8) {
        ...
        if idx < MAX_KNOBS && steps != 0 {
            ...
            log::info!("Knob {:?} turned {} ({} steps)", knob, direction, steps.abs());
        }
    }
    ...
    pub fn build_knob_press_report(&self) -> Vec<u8> {
        ...
        for i in 0..MAX_KNOBS {
            report[5 + i] = if self.knob_pressed[i].load(Ordering::Relaxed) { 0x01 } else { 0x00 };
        }
...
impl Clone for PlusInputState {
    fn clone(&self) -> Self {
        Self {
            ...
            knob_pressed: std::array::from_fn(|i| AtomicBool::new(self.knob_pressed[i].load(Ordering::Relaxed))),
            knob_rotation: std::array::from_fn(|i| AtomicI8::new(self.knob_rotation[i].load(Ordering::Relaxed))),
