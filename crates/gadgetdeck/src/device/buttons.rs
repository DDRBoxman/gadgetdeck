...
impl ButtonState {
    ...
    pub fn build_input_report(&self) -> Vec<u8> {
        let num_buttons = self.model.num_buttons() as usize;

        match self.model {
            StreamDeckModel::Mini => {
                // Module 6 format: [Report ID, button states...]
                let hid_config = self.model.hid_config();
                ...
                for i in 0..num_buttons {
                    if i + 1 < report.len() {
                        report[i + 1] = if self.buttons[i].load(Ordering::Relaxed) {
                            0x01
                        } else {
                            0x00
                        };
            ...
            StreamDeckModel::Pedal => {
                ...
                for i in 0..num_buttons.min(3) {
                    report[4 + i] = if self.buttons[i].load(Ordering::Relaxed) {
                        0x01
                    } else {
                        0x00
                    };
            ...
            StreamDeckModel::Mk2 | StreamDeckModel::Xl => {
                ...
                for i in 0..num_buttons {
                    report[header_size + i] = if self.buttons[i].load(Ordering::Relaxed) {
                        0x01
                    } else {
                        0x00
                    };
    ...
    fn clone(&self) -> Self {
        Self {
            buttons: std::array::from_fn(|i| {
                AtomicBool::new(self.buttons[i].load(Ordering::Relaxed))
            }),
            model: self.model,
...
mod tests {
    ...
    fn test_input_report_format_mini() {
        ...
        let report = state.build_input_report();
